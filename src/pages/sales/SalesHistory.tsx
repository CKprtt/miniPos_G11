...
export default function SalesHistory() {
  ...
  const loadSales = async (uid: string) => {
    try {
      ...
      alert(`โหลดประวัติการขายไม่สำเร็จ: ${error.code || error.message}`);
      setSalesData([]);
    } finally {
      setLoading(false);
    }
  ...
  useIonViewWillEnter(() => {
    const unsub = onAuthStateChanged(auth, async (user) => {
      if (!user) { setSalesData([]); setLoading(false); return; }
                      ...
                      <div className={`custom-table-row ${index % 2 !== 0 ? "even" : ""}`} key={item.id}>
                        ...
                        <div className="td col-detail">
                          <span className="detail-link">
                            {item.items.length > 0 ? item.items.map(x => `${x.name} x${x.qty}`).join(", ") : "-"}
                          </span>
                        </div>
                      </div>
                    );
